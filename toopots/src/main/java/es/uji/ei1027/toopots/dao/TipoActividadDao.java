...
public class TipoActividadDao {
	...
	public List<TipoActividad> getTipoActividadesInstructor(String idInstructor) {
		try {
			return jdbcTemplate.query(
					"select tipoactividad.* FROM tipoactividad"
					+ " INNER JOIN acredita ON tipoactividad.idtipoactividad=acredita.idtipoactividad"
					+ " INNER JOIN acreditacion ON acredita.idacreditacion=acreditacion.idacreditacion"
					+ " WHERE acreditacion.idinstructor=?;",
					new TipoActividadRowMapper(), idInstructor);
		} catch (EmptyResultDataAccessException e) {
			return new ArrayList<TipoActividad>();
		}
	}
