...
public class ClienteController {
	...
	public String processAñadirSubmit(Model model, HttpSession session, @ModelAttribute("reserva") Reserva reserva,
		...
		redirectAttributes.addFlashAttribute("message",
				"Se ha reservado la actividad \"" + actividad.getNombre() + "\"");
	...
	public String añadirComentario(HttpSession session, Model model, @PathVariable String idActividad,
			@RequestParam("valoracion") int valoracion, @RequestParam("comment") String comment,
	...
	public String prefiereUpdate(@RequestParam(value = "prefiere", required = false) List<Integer> new_preferencias,
	...
	public String processAddSubmit(@ModelAttribute("cliente") Cliente cliente, BindingResult bindingResult,
		...
		if (cliente.getIdCliente().length() != 9 || Character.isLetter(cliente.getIdCliente().charAt(8)) == false) {
			redirectAttributes.addFlashAttribute("errorDNI",
					"Formato de DNI incorrecto, recuerda que debes introducir 8 dígitos y 1 letra");
			redirectAttributes.addFlashAttribute("alertClass", "alert-danger");
